...
function prov.connect(host,port)
  ...
  if web==nil then error("TODO") end
  ...
  if method==nil then error("TODO") end
  local ctx=ffi.gc(lib.SSL_CTX_new(method),lib.SSL_CTX_free)
  if ctx==nil then error("TODO") end
  ...
  repeat
    ...
    if stat~=1 and lib.SSL_get_error(ssl,stat)==2 or lib.SSL_get_error(ssl,stat)==3 then sync(true) end
  until stat == 1
  ...
  function conn:receive(data)
    ...
    while true do
      ...
      elseif rec_buf:len()>0 then
        rblc=rblc+1
        if rblc>100 then error("Buffer not shrinking",2) end
