...
local function answer(socket)
  ...
  if not (method and page) then return invalid(1) end
  local headers={}
  repeat
    local line=getline()
    if not line then return invalid(2) end
  ...
  if headers["content-length"] then
    ...
    local start=os.time()
    while d:len()<len do
      if (os.time()-start)>3 then return invalid(3) end
      if buf:len()>0 then
        local len=len-d:len()
        d=d..buf:sub(1,len)
        buf=buf:sub(len+1)
  ...
  for k,v in pairs(headers) do
    ...
    k=k:gsub("%-.",function(a) return a:upper() end)
    k=k:gsub("^.",function(a) return a:upper() end)
  ...
  elseif mode=="normal" then
    while true do
      local cur=content()
      --print("SEND",cur:len())
      if not cur then socket:close() return end
      if cur:len()==0 then break end
      socket:send(cur)
    end
...
function getpage(args)
  if pages[args.page] then
    ...
    if pages[args.page].single then pages[args.page]=nil end
