    ...
    return function(socket)
      scheduler.addthread(function()
        ...
        ws.send=function(sd)
          ...
          if raw then socket:send(sd) return end
          local len=sd:len()
          local data=""
          if len>65535 then
        ...
        local buf=""
        local function read()
          if not open then return string.char(0) end
          while buf:len()==0 do
            local d=socket:receive()
            if not d then open=false return string.char(0) end
            buf=buf..d
          end
          local c= buf:sub(1,1)
          buf=buf:sub(2)
        ...
        function receive()
          if not open then return nil,"closed" end
          if raw then return read() end
          local type,len=struct.unpack(">BB",read()..read())
          if not open then return nil,"closed" end
          if len<128 then error("NEED MASKING") end
          len=len-128
          if len==126 then
