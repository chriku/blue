...
function matrix.connect(host, user, pass)
  ...
  do -- Check for r0.5.0
    ...
    for _, version in ipairs(data.versions) do
      if version == "r0.5.0" then
        good_version = true
      end
    end
    if not good_version then
      return nil, "Server doesn't support Version r.0.5.0"
    end
  end
  ...
  do -- Check for password feature
    ...
    if not data.flows then
      return nil, data.errcode
    end
    local has_password = true
    for _, flow in ipairs(data.flows) do
      if flow.type == "m.login.password" then
        has_password = true
      end
    end
    if not has_password then
      return nil, "Server doesn't support Password"
    end
  end
  ...
  local M = {}
  M.__index = M
  local handle = setmetatable({on_invite = false, on_room_joined = false}, M)
  ...
  do
    ...
    function create_room_handle(id, state, is_first)
      if room_handles[id] then
        return room_handles[id]
      end
      local room_handle = {}
      ...
      function M.send_message_event(type, content)
        ...
        repeat
          ...
          if msg.retry_after_ms then
            scheduler.sleep(msg.retry_after_ms / 1000)
          end
          if msg.event_id then
            return true
          end
