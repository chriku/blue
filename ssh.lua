...
function ssh.connect(host,port,username,socket_provider)
  ...
  local session=ffi.gc(libssh2.session_init_ex(nil,nil,nil,nil),function(o) ffi.C.close(skts[0]) assert(ffi.C.close(skts[1])==0) libssh2.session_free(o) end)
  ...
  local function execp(func,...)
    return exec(function(...)
      ...
      if ptr==nil then return err end
      return ptr
    end,...)
  end
  ...
  function prov.connect(host,port)
    ...
    function skt:receive()
      if exec(libssh2.channel_eof,channel)~=0 then return nil,"closed" end
