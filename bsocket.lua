...
if package.loaded.copas then
  ...
  socket.bind=function(host,port,cb)
    ...
    return {local_port=function() local _,port=server:getsockname() return tonumber(port) end}
  end
...
--print(len,err,err.code,err.code~="WOULD_BLOCK")
    ...
    while dr()<0 do
      ...
      function kf.kf()
        if ok then ok=false good=false scheduler.resume(me,src) end
      end
      src:set_callback(function()
        if ok then ok=false scheduler.resume(me,src) end
      end,function() print("X") end)
      src:attach(nil)
      coroutine.yield()
      src:destroy()
    end
  ...
  socket.connect=function(host,port)
    ...
    coroutine.yield()
    if not conn then print("CE",err,err.message) return nil,err.message end
    return wrap(conn)
  end
  socket.bind=function(host,port,cb)
    ...
    asy()
    --server:start()
    return {local_port=function()return port end}
  end
  return socket
else
